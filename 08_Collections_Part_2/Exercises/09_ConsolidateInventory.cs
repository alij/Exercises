...
namespace Exercises
{
    public partial class Exercises
    {
        ...
        {
            Dictionary<string, int> finalInventory = new Dictionary<string, int>();
            
            foreach (KeyValuePair<string, int> item in remoteWarehouse)
            {
                if (mainWarehouse.ContainsKey(item.Key))
                {
                    mainWarehouse[item.Key] += remoteWarehouse[item.Key];
                }
                else
                {
                    mainWarehouse.Add(item.Key, remoteWarehouse[item.Key]);
                }
            } 
